$(document).ready(() => {
    ...
    // $('h4').eq(4).text('eq()에 의해서 선택되었습니다.'); // text() -> innerText와 같은 역할
    // $('h4:eq(4)').text('eq()에 의해서 선택되었습니다.');
    $('h4').filter(':eq(4)').text('eq()에 의해서 선택되었습니다.');

    // value는 사용자에게 입력받을 때 사용한다.

    // $('.test').odd() : 홀수 인덱스만 조회
    // $('.test').even() : 짝수 인덱스만 조회
    
    // 선택된 요소들 중 홀수 인덱스에 요소들을 가져와서 스타일 변경
    // $('.test').odd().css({'color' : 'white', 'background-color' : 'skyblue'});
    // $('.test:odd').css({'color' : 'white', 'background-color' : 'skyblue'});
    $('.test').filter(':odd').css({'color' : 'white', 'background-color' : 'skyblue'});
    
    // 선택된 요소들 중 짝수 인덱스에 요소들을 가져와서 스타일 변경
    // $('.test').even().css({'color' : 'white', 'background-color' : 'lavender'});
    // $('.test:even').css({'color' : 'white', 'background-color' : 'lavender'});
    $('.test').filter(':even').css({'color' : 'white', 'background-color' : 'lavender'});

    // 선택된 요소들 중 특정 요소가 아닌 요소들을 선택해서 스타일 변경
    // $('h4').not('.test').css({'color' : 'white', 'background-color' : 'gainsboro'});
    // $('h4:not(.test)').css({'color' : 'white', 'background-color' : 'gainsboro'});
    $('h4').filter(':not(.test)').css({'color' : 'white', 'background-color' : 'gainsboro'});
    ...
    // $('span').parents(); 모든 부모 조회

    // 2. Ancestors 메소드
    // span 요소의 부모 요소에 스타일 변경
    // $('span').parent().css({border: '2px solid', color: 'cornflowerblue'});

    // li 요소의 모든 상위 요소에 스타일 변경
    // $('li').parents().css({border: '2px dotted', color: 'mediumpurple'});

    // li 요소의 모든 요소 중 div만 스타일 변경
    // $('li').parents('div').css({border: '4px dotted', color: 'mediumpurple'});

    // span 요소부터 div 요소 이전까지 상위 요소의 스타일 변경
    // $('span').parentsUntil('div').css('background-color', 'lavender');


    // 3. Descendants 메소드
    let style1 = {border: '2px solid', color: 'tomato'};
    let style2 = {border: '2px solid', color: 'orange'};
    let style3 = {border: '2px solid', color: 'yellow'};
    let style4 = {border: '2px solid', color: 'green'};
    let style5 = {border: '2px solid', color: 'skyblue'};
    ...
    // 클래스 속성값이 wrap인 요소의 자식의 자식 요소 중에 ul 태그(요소)의 스타일 적용
    ...
    // h2 요소의 다음에 오는 모든형제 요소에 스타일 적용
    $('.wrap>h2').nextAll().css('background-color', 'navy');
    ...
    $('.wrap>h2').prevAll().css('background-color', 'violet');
    $('.wrap>h2').prevUntil().css('font-size', '3rem');
    // h2 요소 이전부터 p 요소 이후까지의 모든 형제 요소에 스타일 적용
