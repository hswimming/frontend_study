...
window.onload = () => {
    ...
    btn1.addEventListener('click', () => {
        ...
        // exec('문자열') : 문자열에서 정규표현식 패턴을 만족하는 값이 있으면 값을 리턴한다. 단, 없으면 null을 리턴한다.
        ...
        div1.innerHTML += `str1.split(regExp1) : ${str1.split(regExp1)}<br>`; // -1
        div1.innerHTML += `str2.split(regExp1) : ${str2.split(regExp1)}<br>`; // -1
        
        console.log(str1.split(regExp1));

        // replace(정규표현식, 바꿀 값) : 문자열에서 정규표현식 패턴을 만족하는 값을 바꿀값으로 변경한다.
        div1.innerHTML += `str1.replace(regExp1, '스크립트') : ${str1.replace(regExp1, '스크립트')}<br>`; // -1
        div1.innerHTML += `str1.replace(regExp2, '스크립트') : ${str2.replace(regExp1, '스크립트')}<br>`; // -1
    });
    ...
    btn2.addEventListener('click', () => {
        ...
        div2.innerHTML += `/a/ : ${str.replace(/a/, '($&)')}<br>`; // a라는 문자를 찾겠다는 뜻
    ...
    btn3.addEventListener('click', () => {
        ...
        // $는 문자열의 마지막을 의미한다.
        str = 'Javascript\nJquery\nShellscript\nAjax';
        div3.innerHTML += `/^j/ig : ${str.replace(/^j/ig, '($&)')}<br>`; // 문자열의 시작 (j만)
        div3.innerHTML += `/^j/igm : ${str.replace(/^j/igm, '($&)')}<br>`; // 문자열의 시작, 줄바꿈도 적용 (j만)
        ...
        div3.innerHTML += `/[0-9 A-Z]/g : ${str.replace(/[0-9 A-Z]/g, '($&)')}<br>`; // 공백도 하나의 문자로 구분한다. (, 도 마찬가지)
        
        // 임의의 문자열 검색
        // .은 임의의 문자 한 개를 의미하고 문자 내용은 무엇이든 상관없다.
        div3.innerHTML += `/.../g : ${str.replace(/.../g, '($&)')}<br><br>`; // 공백도 포함한다.
        
        // 문자 클래스
        str = 'A1 B2 C3 d_4 e:5 ` ffgg77--__-- \t가\t나\n다';

        // \d : 숫자를 의미한다. [0-9] 와 같다.
        // /\\d/g -> 출력구문에서는 2개 써야 1개로 보임, 표현식에서는 1개만 써도 된다.
